...
export const useMovieStore = defineStore({
  ...
  actions: {
    async loadMovies (payload) {
      ...
      const { data, error, pending } = await useApiFetch('/movies',
        {
    ...
    async loadGenres () {
      try {
        const { data, error, pending } = await useApiFetch("movie_genres")
        this.genres = await data.value
      } catch (error) {
    ...
    async loadMovieRanks (payload) {
      ...
      this.movieRanks = await data.value.data
    },
